...
export function ActiveChatProvider({ children }: { children: ReactNode }) {
  ...
  const initialMessages: ChatMessage[] = isNewChat
    ? []
    : (chatData?.messages ?? []);
  const { visibilityType: visibility } = useChatVisibility({
    chatId,
    initialVisibilityType: isNewChat
      ? "private"
  ...
  const { studentContext } = usePlacement();
  const studentContextRef = useRef(studentContext);
  useEffect(() => {
